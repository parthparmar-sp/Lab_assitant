  ...
  function Auth() {
    ...
    const handleLogin = async ()=>{
      ...
      try {
        ...
        const response= await axios.post(LOGIN_ROUTES,data,{withCredentials:true, headers: { "Content-Type": "application/json" },});
          ...
          <div className="flex flex-col items-start justify-start w-full p-10 space-y-6">
            <Tabs className="w-full" defaultValue="login">
              ...
              <TabsContent value='login' className="space-y-4 mt-4">
                <Input placeholder='Enter Your Email' className="rounded-2xl p-4 border border-gray-300 focus:border-blue-800 focus:ring-2 focus:ring-blue-800 transition-all duration-300" value={Email} onChange={(e) => setEmail(e.target.value)} />
                <Input placeholder="Enter Your Password" type="password" className='rounded-2xl p-4 border border-gray-300 focus:border-blue-800 focus:ring-2 focus:ring-blue-800 transition-all duration-300' value={Password} onChange={(e) => setPassword(e.target.value)} />
                <button className="w-full mt-4 p-4 bg-blue-800 text-white rounded-2xl font-semibold hover:bg-blue-900 transition-all duration-300" onClick={handleLogin}>Login</button>
              </TabsContent>
